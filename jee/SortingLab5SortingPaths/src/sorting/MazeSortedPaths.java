...
public class MazeSortedPaths {
	...
	/**
	 * List of paths through the maze, sorted from shortest (first position) to
	 * longest (last position)
	 */
	...
	public void sort() {
		/**
		 * The sort method in class Collection orders a collection of elements based on
		 * the Comparator provided as second argument.
		 */
		sortedPaths.sort((a, b) -> a.size() < b.size() ? -1 : a.size() == b.size() ? 0 : 1);
	}
	...
	public ArrayList<Path> getShortestPaths() {
		...
		for (int pos = 0; pos < sortedPaths.size() && sortedPaths.get(pos).size() == minLength; pos++) {
	...
	public ArrayList<Path> getLongestPaths() {
		...
		for (int pos = sortedPaths.size() - 1; pos >= 0 && sortedPaths.get(pos).size() == maxLength; pos--) {
