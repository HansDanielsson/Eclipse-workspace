...
public class Maze {
	...
	public int[] getNeighbourCoords(int row, int col, Movement mov) {
		...
		int[] newCoords = new int[] { currentCoords[0] + mov.vShift(), currentCoords[1] + mov.hShift() };
		if (newCoords[0] < 0 || newCoords[0] >= maze.length || newCoords[1] < 0 || newCoords[1] >= maze[0].length) {
			// invalid position
			newCoords = null;
		}
		return newCoords;
