...
/**
 * Models the complete set of potential paths from a starting point. Notice that
 * it grows exponentially, and thus may surpass space/time limits pretty soon.
 * 
...
public class PathsTree {
	 ...
	 * 
	 * if (root != null) { root.stepForward(); for (PathsTree child : root.children)
	...
	private void print(int indent) {
		if (root != null) {
			...
			/*
			 * // Alternative for indentation char[] chars = new char[indent];
			 * Arrays.fill(chars, ch); String s = new String(chars);
			 */
		...
		if (root != null) {
			if (root.children.isEmpty()) { // this is a leaf
				if (root.maze.getPosStatus(root.pos) == MazeStatus.GOAL) {
					/*
					 * if this is a leaf and it is the GOAL the path finish successfully: build path
					 * and add it to the list of solutions
					 */
					...
					while (p != null) {
						/*
						 * Notice that cells must be reversed when building the path as we are
						 * traversing backwards from the goal towards the initial position
						 */
				...
				/*
				 * if it is a leaf but it is not the GOAL this path leads to a dead end: ignore
				 * and continue exploring other branches
				 */
	...
	/**
	 * Models each node in the paths tree Contains the position of the current step
	...
	private class PathsTreeNode {
		...
		/*
		 * // UNNEEDED PathsTreeNode(int row, int col, MazePosition parent) { this.pos =
		 * new MazePosition(row, col, parent); this.children = new
		 * ArrayList<PathsTree>(); }
		 */
		...
		void stepForward() {
			...
			if (maze.getPosStatus(pos) != MazeStatus.GOAL) {
				...
				for (Movement mov : Movement.values()) {
					...
					if ((next != null)
							&& (maze.getPosStatus(next) == MazeStatus.OPEN
									|| maze.getPosStatus(next) == MazeStatus.GOAL) // valid position
