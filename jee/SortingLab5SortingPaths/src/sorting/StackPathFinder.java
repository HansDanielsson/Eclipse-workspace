...
/**
 * Class StackPathFinder implements an algorithm for finding a path using a
 * Stack
 * 
...
public class StackPathFinder implements PathFinder {
	 ...
	 * using a Stack means retrieving elements in reverse order. Thus, most
	 * promising directions are inserted last in the Stack in order to get them
	 * explored first.
	 */
	private static final Movement[] DIRS_TO_EXPLORE = new Movement[] { Movement.LEFT, Movement.UP, Movement.DOWN,
			Movement.RIGHT };
	...
	public Path findPath(Maze maze) {
		...
		Stack<MazePosition> posToExplore = new Stack<>();
		...
		while (!posToExplore.isEmpty()) {
			...
			case OPEN:
				...
				/*
				 * ** IMPORTANT: The exploration order determines the path found even forcing to
				 * traverse every cell in the maze
				 */
