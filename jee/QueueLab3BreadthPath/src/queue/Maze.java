...
public class Maze {
	...
	public Maze(int width, int height) {
		mazest = new MazeStatus[height][width];
		for (int row = 0; row < height; row++) {
			for (int col = 0; col < width; col++) {
				mazest[row][col] = MazeStatus.OPEN;
			}
		}
	}
	...
	public Maze(String sMaze) {
		mazest = stringToMaze(sMaze);
	}

	/**
	 * Returns the MazeStatus value corresponding to the given cell, specified by
	 * its row and column
	 */
	public MazeStatus getPosStatus(int row, int col) {
		return mazest[row][col];
	}
	...
	public MazeStatus getPosStatus(MazePosition pos) {
		return mazest[pos.getCoords()[0]][pos.getCoords()[1]];
	}
	...
	public void setPosStatus(MazePosition pos, MazeStatus newStatus) {
		mazest[pos.getCoords()[0]][pos.getCoords()[1]] = newStatus;
	}

	/**
	 * Creates and returns an String with the text-based representation of the given
	 * Maze
	 */
	public String toString() {
		String s = null;
		if (mazest != null && mazest[0] != null) {
			s = "";
			for (int r = 0; r < mazest.length; r++) {
				for (int c = 0; c < mazest[0].length; c++) {
					s = s.concat(String.valueOf(mazest[r][c].text()));
				}
	...
	/**
	 * Transforms a text-based Maze into a bidimensional array of positions with the
	 * corresponding status. It creates the array maze and assign to each of its
	 * cells the corresponding status value based on the given text representation.
	 */
	...
	/**
	 * Calculates the destination position in the Maze given an starting position
	 * (row, col) and a Movement (mov)
	 * 
	 * @returns the coordinates of the next position, if its is a valid position.
	 *          Returns null if the destination position is outside the limits of
	 *          the array.
	 */
	...
	public int[] getNeighbourCoords(int row, int col, Movement mov) {
		int[] newCoords = new int[] { row + mov.vShift(), col + mov.hShift() };
		if (newCoords[0] < 0 || newCoords[0] >= mazest.length || newCoords[1] < 0 || newCoords[1] >= mazest[0].length) {
			// invalid position
			newCoords = null;
		}
		return newCoords;
	 ...
	 * initial status of the position to visit is OPEN or VISITED (can step into the
	 * position), change it to VISITED. If the initial status of the position to
	 * visit is GOAL, does not change it so that the goal keeps displaying in the
	 * maze. If the initial status of the position to visit is OBSTACLE or the
	 * position is outside the limits of the Maze, it is an invalid move, stop and
	 * finish. Empties the received path as it traverses it.
	 * 
	 */
